...
| Public code and MIT/Apache license | Public himanshu748/scamshield; GitHub detects Apache-2.0 | Publish and verify the tested release commit, including assets and lockfiles |
...
ScamShield targets **Good Neighbor Agents**. The demonstrated loop is:
Suspicious message text → Inspect redacted message and checks → Apply deterministic risk rules → Review evidence and uncertainty → Local report for a trusted helper.

The group benefit is a reviewed person-to-helper handoff. There is no shared inbox, authenticated sender check, group deployment, or demonstrated nonprofit adoption. Prove the helper handoff with a real user review before claiming community impact.
