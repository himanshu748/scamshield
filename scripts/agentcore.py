...
def update_code(*, allow_paid=False):
    """Update only the packaged code of the recorded runtime, preserving its configuration."""
    if not allow_paid:
        raise SystemExit("Runtime code updates require --allow-paid")
    state = json.loads(STATE.read_text())
    session = boto3.Session(region_name=REGION)
    account = session.client("sts").get_caller_identity()["Account"]
    if not state["arn"].startswith(f"arn:aws:bedrock-agentcore:{REGION}:{account}:runtime/"):
        raise SystemExit("Recorded runtime does not belong to this account and region")
    control = session.client("bedrock-agentcore-control")
    current = control.get_agent_runtime(agentRuntimeId=state["id"])
    if current["status"] != "READY" or current["agentRuntimeArn"] != state["arn"]:
        raise SystemExit("The recorded runtime must be READY before updating")
    archive = BUILD / "deployment.zip"
    digest = hashlib.sha256(archive.read_bytes()).hexdigest()
    key = f"runtime/{digest}.zip"
    session.client("s3").upload_file(
        str(archive), state["bucket"], key, ExtraArgs={"ServerSideEncryption": "AES256"}
    )
    settings = {
        k: current[k]
        for k in (
    ...
    response = control.update_agent_runtime(
        agentRuntimeId=state["id"],
        agentRuntimeArtifact={
            "codeConfiguration": {
                "code": {"s3": {"bucket": state["bucket"], "prefix": key}},
                "runtime": "PYTHON_3_12",
                "entryPoint": ["agentcore_main.py"],
            }
        },
    ...
    state["previous_code"] = current["agentRuntimeArtifact"]
    state.update(key=key, sha256=digest, version=response["agentRuntimeVersion"])
    STATE.write_text(json.dumps(state, indent=2) + "\n")
    print(json.dumps({"updated": True, "version": state["version"], "sha256": digest}))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "command",
        choices=["package", "preflight", "deploy", "update-code", "status", "set-log-retention"],
    )
    parser.add_argument(
        "--allow-paid",
        action="store_true",
    ...
    QUOTA_CODE = args.quota_code or QUOTA_CODE
    {
        ...
        "update-code": lambda: update_code(allow_paid=args.allow_paid),
        "status": status,
